/**
 * CircularGallery - Vanilla JS port of the React Bits "Circular Gallery" component
 * Uses the OGL WebGL library for premium 3D image bending and interactive rotation.
 */

// Alias global lowercase ogl to window.OGL if needed
...
// Main CircularGallery API Class
class CircularGallery {
  constructor(container, options = {}) {
    if (!window.OGL) {
      console.error('CircularGallery error: OGL WebGL library is not loaded. Please make sure ogl.umd.js is included.');
      return;
    }
    ...
    const bend = options.bend !== undefined ? options.bend : 1.5;
    const textColor = options.textColor || '#ffffff';
    const borderRadius = options.borderRadius !== undefined ? options.borderRadius : 0.05;
    const font = options.font || 'bold 24px "Plus Jakarta Sans", sans-serif';
    const fontUrl = options.fontUrl || null;
    
    this.scrollSpeed = options.scrollSpeed !== undefined ? options.scrollSpeed : 2.5;
    this.scrollEase = options.scrollEase !== undefined ? options.scrollEase : 0.06;
    this.itemWidth = options.itemWidth || 850;   // default bigger: 850 instead of 700
    this.itemHeight = options.itemHeight || 1150; // default bigger: 1150 instead of 900
  ...
  createMedias(items, bend, textColor, borderRadius, font) {
    const defaultItems = [
      ...
      { image: 'https://picsum.photos/seed/2/800/600', text: 'Desk' }
    ];
    const galleryItems = items && items.length ? items : defaultItems;
    
    // Duplicate items to ensure smooth wrap loop in circular geometry
  ...
  onWheel(e) {
    // Only intercept wheel events if the mouse is hovered over the gallery container
    const isHovered = this.container.contains(e.target);
    if (!isHovered) return;
  ...
  addEventListeners() {
    ...
    // Pass passive: false to allow e.preventDefault() in wheel listener
